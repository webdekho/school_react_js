...
class Staff extends API_Controller {
    ...
    // Get staff list for dropdown (Staff version)
    public function admin_staff_dropdown() {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
    ...
    // ==================== ATTENDANCE MANAGEMENT (ADMIN FUNCTIONS) ====================

    /**
     * Get attendance records with filters (Staff version of admin function)
    ...
    /**
     * Get class attendance for a specific date (Staff version)
     * GET /api/staff/class_attendance/{grade_id}/{division_id}/{date}
     */
    public function admin_class_attendance($grade_id, $division_id, $date) {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
    ...
    /**
     * Mark attendance (Staff version)
     * POST /api/staff/mark_attendance
     */
    public function admin_mark_attendance() {
        if ($this->input->method() !== 'post') {
            $this->send_error('Method not allowed', 405);
            return;
        }
    ...
    /**
     * Get attendance statistics (Staff version)
     * GET /api/staff/attendance_stats
     */
    public function admin_attendance_stats() {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
    ...
    /**
     * Get low attendance students (Staff version)
     * GET /api/staff/low_attendance_students
     */
    public function admin_low_attendance_students() {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
    ...
    /**
     * Delete attendance record (Staff version)
     * DELETE /api/staff/attendance/{id}
     */
    public function admin_delete_attendance($id) {
        if ($this->input->method() !== 'delete') {
            $this->send_error('Method not allowed', 405);
            return;
        }
    ...
    /**
     * Check in - Staff marks their own attendance
     * POST /api/staff/check_in
     */
    public function check_in() {
        ...
        try {
            $staff_id = $this->user['id'];
            $result = $this->Staff_attendance_model->check_in($staff_id);
            ...
            $this->send_response(null, 'Checked in successfully', 201);

        } catch (Exception $e) {
            log_message('error', 'Check-in error: ' . $e->getMessage());
            $this->send_error('Failed to check in', 500);
        }
    }

    /**
     * Check out - Staff marks their checkout
     * POST /api/staff/check_out
     */
    public function check_out() {
        ...
        try {
            $staff_id = $this->user['id'];
            $result = $this->Staff_attendance_model->check_out($staff_id);
            ...
            $this->send_response(null, 'Checked out successfully');

        } catch (Exception $e) {
            log_message('error', 'Check-out error: ' . $e->getMessage());
            $this->send_error('Failed to check out', 500);
        }
    }

    /**
     * Get own attendance for a specific date
     * GET /api/staff/my_attendance/{date}
     */
    public function my_attendance($date) {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
        ...
        try {
            $staff_id = $this->user['id'];
            $attendance = $this->Staff_attendance_model->get_attendance_by_date($staff_id, $date);

            $this->send_response($attendance);

        } catch (Exception $e) {
            log_message('error', 'Get my attendance error: ' . $e->getMessage());
            $this->send_error('Failed to retrieve attendance', 500);
        }
    }

    /**
     * Get own attendance history
     * GET /api/staff/my_attendance_history
     */
    public function my_attendance_history() {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
        ...
        try {
            ...
            $history = $this->Staff_attendance_model->get_staff_history($staff_id, $month);

            $this->send_response($history);

        } catch (Exception $e) {
            log_message('error', 'Get my attendance history error: ' . $e->getMessage());
     ...
     * GET /api/staff/my_attendance_stats
     */
    public function my_attendance_stats() {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
        ...
        try {
            ...
            $this->send_response($stats);

        } catch (Exception $e) {
            log_message('error', 'Get my attendance stats error: ' . $e->getMessage());
            $this->send_error('Failed to retrieve attendance statistics', 500);
        }
    }
     ...
     * GET/POST /api/staff/vision_statements
     * PUT/DELETE /api/staff/vision_statements/:id
     */
    public function vision_statements($id = null) {
        switch ($this->input->method()) {
            case 'get':
                if ($id) {
                    // Get specific vision statement
                    $vision = $this->Vision_statement_model->get_by_id($id);
                    if (!$vision) {
                        $this->send_error('Vision statement not found', 404);
                        return;
                    }
                    
                    // Check if current staff can view this vision
                    if ($vision['staff_id'] != $this->user['id']) {
                    ...
                    // Get all vision statements for current staff
                    $limit = $this->input->get('limit') ?: 10;
                    $offset = $this->input->get('offset') ?: 0;
                    $grade_id = $this->input->get('grade_id');
                    ...
                    $visions = $this->Vision_statement_model->get_all($limit, $offset, $filters);
            ...
            case 'post':
                // Create new vision statement
                $input = $this->validate_input([
                    'grade_id' => 'integer',
                    'vision' => 'required|max_length[5000]'
                ]);
                
                if (!$input) return;
                
                $input['staff_id'] = $this->user['id'];
                
                try {
                    $vision_id = $this->Vision_statement_model->create_vision_statement($input);
                    if ($vision_id) {
                        $this->send_response(['id' => $vision_id], 'Vision statement created successfully', 201);
                    } else {
            ...
            case 'put':
                ...
                // Check if current staff can edit this vision
                if (!$this->Vision_statement_model->can_edit($id, $this->user['id'])) {
                    $this->send_error('Access denied or vision statement not found', 403);
                    return;
                }
                ...
                if ($this->Vision_statement_model->update_vision_statement($id, $input)) {
                    $this->send_response(null, 'Vision statement updated successfully');
                } else {
            ...
            case 'delete':
                ...
                // Check if current staff can delete this vision
                if (!$this->Vision_statement_model->can_edit($id, $this->user['id'])) {
                    $this->send_error('Access denied or vision statement not found', 403);
                    return;
                }
     ...
     * GET /api/staff/vision_statements_by_grade/:grade_id
     */
    public function vision_statements_by_grade($grade_id = null) {
        if ($this->input->method() !== 'get') {
            $this->send_error('Method not allowed', 405);
            return;
        }
        ...
        try {
            $visions = $this->Vision_statement_model->get_by_grade($grade_id);
            $this->send_response($visions);
        } catch (Exception $e) {
            log_message('error', 'Get vision statements by grade error: ' . $e->getMessage());
