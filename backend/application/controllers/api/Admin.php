...
class Admin extends API_Controller {
    ...
    public function __construct() {
        ...
        // Ensure only admin/staff with admin permissions can access
        if (!$this->require_user_type(['admin', 'staff'])) {
            return;
        }
    }
    ...
    public function grades_dropdown() {
        ...
        if (!$this->require_permission('grades')) {
            return;
        }
    ...
    public function divisions($id = null) {
        if (!$this->require_permission('divisions')) {
            return;
        }
    ...
    // Get semester fees for grade and division
    public function semester_fees() {
        ...
        log_message('debug', 'Semester fees request - grade_id: ' . $grade_id . ', division_id: ' . $division_id . ', academic_year_id: ' . $academic_year_id);
        
        // More specific validation
        if (empty($grade_id) || empty($division_id) || empty($academic_year_id)) {
            $this->send_error('grade_id, division_id, and academic_year_id are required', 400);
            return;
        }
        
        $this->load->model('Student_model');
        $semester_fees = $this->Student_model->get_semester_fees_for_grade_division($grade_id, $division_id, $academic_year_id);
    ...
    public function academic_years($id = null) {
        if (!$this->require_permission('academic_years')) {
            return;
        }
    ...
    public function parents($id = null) {
        if (!$this->require_permission('parents')) {
            return;
        }
    ...
    public function staff($id = null) {
        if (!$this->require_permission('staff')) {
            return;
        }
        ...
        switch ($this->input->method()) {
            case 'get':
                if ($id) {
                    ...
                    $structures = $this->Fee_structure_model->get_fee_structures_paginated($limit, $offset, $search, $academic_year_id, $grade_id, $category_id);
                    $total = $this->Fee_structure_model->count_fee_structures($search, $academic_year_id, $grade_id, $category_id);
            ...
            case 'get':
                if ($id) {
                    try {
                        ...
                        // Log the collection data for debugging
                        log_message('debug', 'Collection data for ID ' . $id . ': ' . json_encode($collection));
    ...
    public function verify_fee_collection($id) {
        ...
        if (!$this->require_permission('fees') || $this->user['user_type'] !== 'admin') {
    ...
    public function announcements($id = null) {
        if (!$this->require_permission('announcements')) {
            return;
        }
