...
class API_Controller extends CI_Controller {
    ...
    private function authenticate() {
        // Temporarily allow access to fee_categories_dropdown for debugging
        if ($this->uri->segment(3) === 'fee_categories' && $this->uri->segment(4) === 'dropdown') {
            $this->user = ['id' => 1, 'user_type' => 'admin']; // Mock admin user for debugging
            $this->user_type = 'admin';
            return; // Skip authentication for this endpoint
        }
        
        // Temporarily allow access to staff wallet endpoints for debugging
        ...
        // Temporarily allow access to fee_collections for debugging - REMOVE IN PRODUCTION
        // if ($this->uri->segment(3) === 'fee_collections') {
        //     $this->user = ['id' => 1, 'user_type' => 'admin']; // Mock admin user for debugging
        //     $this->user_type = 'admin';
        //     return;
        // }
        ...
        $this->user_type = $this->user['user_type'] ?? 'admin';
        
        if (isset($this->user['permissions'])) {
            $this->permissions = json_decode($this->user['permissions'], true);
        } else {
            $this->permissions = ['*']; // Give all permissions for debug
        }
    }
