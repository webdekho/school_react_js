...
class Announcement_model extends CI_Model {
    ...
    /**
     * Fetch announcements visible to a specific parent (targeted to 'all' or 'parent')
     */
    public function get_announcements_for_parent($parent_id, $limit = 10, $offset = 0, $search = null) {
        $this->db->select('a.*, s.name as created_by_name');
        $this->db->from('announcements a');
        $this->db->join('staff s', 'a.created_by = s.id', 'left');
        // Visible if sent and either to all or parent, and if parent is in target_ids when target_type is parent
        $this->db->where('a.status', 'sent');
        $this->db->group_start();
        $this->db->where('a.target_type', 'all');
        $this->db->or_group_start();
        $this->db->where('a.target_type', 'parent');
        // target_ids is JSON text array; match parent id inside JSON safely
        $this->db->like('a.target_ids', '"' . (int)$parent_id . '"');
        $this->db->group_end();
        $this->db->group_end();
        ...
        foreach ($rows as &$row) {
            ...
            $row['channels'] = json_decode($row['notification_channels'] ?: '[]', true);
            $row['target_ids'] = json_decode($row['target_ids'] ?: '[]', true);
            // Derive is_read from delivery status table if exists
            $this->db->select('status');
            $this->db->from('announcement_delivery_status');
            $this->db->where('announcement_id', $row['id']);
            $this->db->where('recipient_type', 'parent');
            $this->db->where('recipient_id', $parent_id);
            $delivery = $this->db->get()->row_array();
            $row['is_read'] = isset($delivery['status']) && $delivery['status'] === 'read';
        }
        
        return $rows;
    }

    public function count_announcements_for_parent($parent_id, $search = null) {
        $this->db->from('announcements a');
        $this->db->where('a.status', 'sent');
        $this->db->group_start();
        $this->db->where('a.target_type', 'all');
    ...
    /**
     * Mark an announcement as read for a recipient
     */
    public function mark_as_read($announcement_id, $recipient_id, $recipient_type = 'parent') {
        // Upsert into delivery status table
        $this->db->where('announcement_id', $announcement_id);
        $this->db->where('recipient_type', $recipient_type);
        $this->db->where('recipient_id', $recipient_id);
        $existing = $this->db->get('announcement_delivery_status')->row_array();
        
        if ($existing) {
            $this->db->where('id', $existing['id']);
            return $this->db->update('announcement_delivery_status', [
                'status' => 'read',
                'updated_at' => date('Y-m-d H:i:s')
            ]);
        } else {
            return $this->db->insert('announcement_delivery_status', [
                'announcement_id' => $announcement_id,
