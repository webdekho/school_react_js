...
const AttendanceManagement = () => {
  ...
  const { data: gradesData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiService.get('/api/staff/grades_dropdown');
  ...
  const { data: divisionsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiService.get('/api/staff/divisions');
      return response.data;
    }
  });
  ...
  const { data: classAttendance, isLoading: loadingClass, refetch: refetchClassAttendance } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiService.get(`/api/staff/class_attendance/${selectedGrade}/${selectedDivision}/${selectedDate}`);
      return response.data;
    },
  ...
  const { data: attendanceHistory, isLoading: loadingHistory } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiService.get(`/api/staff/attendance?${params}`);
      return response.data;
    },
  ...
  const { data: lowAttendanceData, isLoading: loadingLowAttendance } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiService.get('/api/staff/low_attendance_students?threshold=75');
      return response.data;
    },
  ...
  const markAttendanceMutation = useMutation({
    mutationFn: async (data) => {
      return await apiService.post('/api/staff/mark_attendance', data);
    },
    onSuccess: () => {
  ...
  const viewStudentStats = async (studentId) => {
    try {
      const response = await apiService.get(`/api/staff/attendance_stats?student_id=${studentId}`);
